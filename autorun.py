...
output_file = r'C:\images'
folder_path = r'C:\pdf data'  ## give pdf's folder path
...
mycollection = mydatabase['Madugula_Data']
# mycollection = mydatabase['Madugula_Data_image']
myimage = mydatabase['Images_pages1']
...
def covert(x, y, w, h, im2, img_file):
    ...
    if h >= 400 and h < 1000:
        ...
        if x == 0 and y == 0:
            ...
            rec = mycollection.insert_one({'Voter_file_tracker': img_file,
                                           "Voter Image": b,
                                           "Assembly Name": lis[0],
                                           "Section": lis[1],
                                           'details': text,
...
def ima(x, y, w, h, im2, img_file):
    ...
    W = int(w / 3)
    covert2(x + W, y, w, h, im2, img_file)
    covert3(x + W + W, y, w, h, im2, img_file)
...
def img_detect(img_path, img_file):
    lis.clear()
    page(img_path)
    pageimg =pagetobase(img_path)
    rec = myimage.insert_one({
        'Voter_file_tracker': img_path,
        "Assembly Name":lis[0],
        "Sections":lis[1],
        "Voter Image": pageimg,
    })
...
def converting_pdftoimg(pdf_path):
    ...
    print(pdf_path)
    images = convert_from_path(pdf_path, 500, poppler_path=r'C:\Users\vivif\Desktop\convert\poppler-0.68.0\bin')
    name = pdf_path.split("\\")
    print(name)
